import { isPresent, isBlank, BaseException, stringify, CONST_EXPR, StringWrapper } from 'angular2/src/facade/lang';
import { ObservableWrapper } from 'angular2/src/facade/async';
import { ListWrapper, MapWrapper } from 'angular2/src/facade/collection';
import { Injector, Key, Dependency, Binding, ResolvedBinding, NoBindingError, AbstractBindingError, CyclicDependencyError, resolveBindings } from 'angular2/di';
...
export class DirectiveDependency extends DependencyWithVisibility {
    constructor(key, asPromise, lazy, optional, properties, visibility, attributeName, queryDecorator) {
...
export class ProtoElementInjector {
    ...
    static _createDirectiveBindingData(dirBindings, bd, firstBindingIsComponent) {
        ListWrapper.forEach(dirBindings, dirBinding => {
            ListWrapper.push(bd, ProtoElementInjector._createBindingData(firstBindingIsComponent, dirBinding, dirBindings, dirBinding));
        });
    }
    static _createHostInjectorBindingData(dirBindings, bd, firstBindingIsComponent) {
        var visitedIds = MapWrapper.create();
        ListWrapper.forEach(dirBindings, dirBinding => {
            ListWrapper.forEach(dirBinding.resolvedHostInjectables, b => {
                if (MapWrapper.contains(visitedIds, b.key.id)) {
                    throw new BaseException(`Multiple directives defined the same host injectable: "${stringify(b.key.token)}"`);
                }
                MapWrapper.set(visitedIds, b.key.id, true);
                ListWrapper.push(bd, ProtoElementInjector._createBindingData(firstBindingIsComponent, dirBinding, dirBindings, ProtoElementInjector._createBinding(b)));
            });
        });
    }
    static _createBindingData(firstBindingIsComponent, dirBinding, dirBindings, binding) {
